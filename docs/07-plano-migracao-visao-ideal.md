...
| **7** | **Pendências e Justificativas** (4 abas) — fase própria. Desenho consolidado em **`docs/09`** (Sustaining: Conta × Gerência, dono = ponto focal da Gerência; Obras: Projeto/PEP). Schema `fact_explicacao_log` já existe; falta extensão (`gerencia_id`/`universo`/`e_pep_projeto`) + motor da Fila + UI. Bloqueado em 6 validações de negócio com a MRS (docs/09 §6) | alto | MAJOR | negócio |
