...
| **7a.2** | Motor da Fila de Pendências (módulo, sem UI): lê `fact_explicacao_log` (Neon) + Deltas (DuckDB), calcula pendências por universo/`escopo_temporal` recortado pelo escopo do usuário; Sustaining Micro mensal+acumulado (sem threshold), Sustaining Macro só acumulado (threshold), Obras só acumulado (threshold); "só mês fechado"; some/reabre sozinha. Teste numérico. | médio | MINOR | 7a.1 |
